...
public:
  ...
  void onFrame( const Leap::Controller& controller )
  {
	...
	if( thumb.isExtended() && indexFinger.isExtended()){
	  ...
	  double d = 0.0;
	  std::cout << "m_volume->minValue()"<<m_volume->minValue() << std::endl;
	  std::cout << "m_volume->maxValue()"<<m_volume->maxValue() << std::endl;
	  if(distance > 90) {
	    distance = 90;
	    d = m_volume->minValue();
	  }
	    ...
	    {
	      distance = 30;
	      d = m_volume->maxValue()*0.9;
	    }
	  
	  else{
	  d = m_volume->maxValue()-distance*(m_volume->maxValue()/90);
	  }
